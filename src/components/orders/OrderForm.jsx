...
export const OrderForm = observer(({ type, store }) => {
  ...
  const addCustomer = async () => {
    ...
    try {
      const result = await store.addCustomer(newCustomerName)
      setForm(current => ({ ...current, customerId: result.id }))
      setNewCustomerName('')
      setShowCustomer(false)
    } catch (customerError) {
      setError(customerError.message || 'Không thể thêm khách hàng.')
    }
  }
  ...
  const addOrder = async event => {
    ...
    setSaving(true)
    try {
      const result = await store.createOrder({ type, form, items })
      ...
      setError(orderError.message || 'Không thể lưu phiếu.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return <section className="panel order-form">
    ...
    <form className="form-grid" onSubmit={addOrder}>
      ...
      {type === 'Outbound' ? <label className="span-2">Khách hàng<div className="inline-control"><select required value={form.customerId} onChange={e => setForm({ ...form, customerId: e.target.value })}><option value="">Chọn khách hàng</option>{activeCustomers.map(customer => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select>{store.canCreateCustomer() && <button type="button" className="button subtle" title="Thêm khách hàng" onClick={() => setShowCustomer(!showCustomer)}><Icon name="plus" /></button>}</div></label> : <label className="span-2">Nhà cung cấp / đối tác<input value={form.partnerName} onChange={e => setForm({ ...form, partnerName: e.target.value })} placeholder="Tên đối tác (không bắt buộc)" /></label>}
      ...
      {showCustomer && type === 'Outbound' && store.canCreateCustomer() && <div className="quick-add span-2"><b>Thêm khách hàng nhanh</b><div className="inline-control"><input placeholder="Tên khách hàng" value={newCustomerName} onChange={e => setNewCustomerName(e.target.value)} /><button type="button" className="button primary" onClick={addCustomer}>Lưu</button></div></div>}
      <button disabled={saving} className="button primary span-2"><Icon name="plus" /> {saving ? 'Đang lưu…' : 'Lưu phiếu'}</button>
    </form>
  </section>
})
