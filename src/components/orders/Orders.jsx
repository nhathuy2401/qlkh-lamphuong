...
export const Orders = observer(({ type, store }) => {
  ...
  return <><PageHeader eyebrow={type === 'Outbound' ? 'OUTBOUND' : 'INBOUND'} title={type === 'Outbound' ? 'Phiếu xuất kho' : 'Phiếu nhập kho'} description="Tìm kiếm nhanh và kiểm soát toàn bộ lịch sử phiếu." /><OrderFilters type={type} filters={filters} setFilters={setFilters} customers={data.customers} /><div className="orders-layout"><OrderForm type={type} store={store} /><section className="panel"><div className="panel-heading"><div><h3>Kết quả tìm kiếm</h3><span>{orders.length} phiếu phù hợp</span></div></div>{orders.length ? <div className="order-cards">{orders.map(order => <OrderCard key={order.id} order={order} onDelete={setConfirmId} />)}</div> : <Empty title="Không có phiếu phù hợp" text="Thử điều chỉnh bộ lọc hoặc tạo phiếu mới." />}</section></div><DeleteOrderDialog open={Boolean(confirmId)} onCancel={() => setConfirmId(null)} onConfirm={() => { store.deleteOrder(confirmId); setConfirmId(null) }} /></>
})
